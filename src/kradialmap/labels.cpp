...
#include <math.h>           //paintExplodedLabels()

#include <qfont.h>          //paintExplodedLabels()
#include <qfontmetrics.h>   //paintExplodedLabels()
#include <qpainter.h>       //paintExplodedLabels()
#include <qptrlist.h>       //paintExplodedLabels()

#include <kstringhandler.h> //paintExplodedLabels()
...
//FIXME put inside KRadialMap class


struct Label
{
  Label( const KRadialMap::Segment *s, int l ) : segment( s ), lvl( l ), a( segment->start() + (segment->length() / 2) ) { }

  bool tooClose( const int &aa ) const { return ( a > aa - LABEL_ANGLE_MARGIN && a < aa + LABEL_ANGLE_MARGIN ); }

  const KRadialMap::Segment *segment;
...
//friend of FileMap
void KRadialMap::paintExplodedLabels( QPainter &paint ) const
{
  LabelList list;
  ...
  do
  {
...

    {
      ...
      varySizes = Gsettings.varyLabelFontSizes && (range != 0);
      ...
      if( varySizes )
      {
        ...
        unsigned int step = (paint.font().pointSize() - Gsettings.minFontPitch) / range;
        if( step == 0 ) step = 1;

        for( unsigned int x = range + startLevel, y = Gsettings.minFontPitch;
             x >= startLevel;
             y += step, --x )
        {
          sizes[x] = y;
    ...
    KStringHandler kstr; //currently using csqueeze //**** KDE 3.2 has a pixel squeezer
    QFont font;
